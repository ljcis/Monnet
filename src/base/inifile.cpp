...
void CIniFile::TraverseSectionName( std::vector< std::string > &vec_section )
{
    struct ENTRY *pEntry = m_pEntry;            /* Get a pointer to the first Entry */

    while( pEntry != NULL )
    {
        if( pEntry->Type == tpSECTION )
        {
            std::string str_section = pEntry->pText;
            if( str_section[0] == '[' )
            {
                str_section.erase( 0, 1 );
            }

            if( *( str_section.rbegin() ) == ']' )
            {
                str_section.erase( str_section.size() - 1 );
            }

            vec_section.push_back( str_section );
        }
...
void CIniFile::TraverseKeyName( std::string str_section, std::vector< std::string > &vec_key )
{
    struct ENTRY *pEntry = FindSection( str_section.c_str() );
    ...
    while( pEntry != NULL )
    {
        ...
        {
            std::string str_key = pEntry->pText;
            str_key = str_key.substr( 0, str_key.find_first_of( '=' ) );

            vec_key.push_back( str_key );
        }
...
{  
    ...
    if(iFile.ReadInt ("IO-Port", "Com", 66)==11112)  printf("\n\ngggggggggggggggggggggggg\n\n");  
